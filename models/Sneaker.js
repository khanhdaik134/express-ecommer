...
class Sneaker extends Model {
  ...
  getAll(search, limit, offset, orderBy, callback){
    ...
    let queryStr = `SELECT ${this.table}.*,brands.title as brand FROM ${this.table} INNER JOIN brands ON brands.id = ${this.table}.brand_id ${searchStr} ${orderByStr} LIMIT ${limit || 10} OFFSET ${offset || 0}`;
